(function(b) {
    ...
    b.extend(b.jqx._jqxGrid.prototype, {
                    ...
                    var q = function() {
                        ...
                        if (g._gridRenderElement) {
                            ...
                            d[b.camelCase(t)](function() {
                                b(g._gridRenderElement).hide("slow",
                                function() {
        ...
        pincolumn: function(d) {
            this._setcolumnproperty(d, "pinned", 'left')	//默认的靠左固定列
        },
        unpincolumn: function(d) {
            this._setcolumnproperty(d, "pinned", null)	//不固定列
        },
        ...
        _updatecolumnwidths: function() {	//更新表头的宽度
            ...
            function(q, u) {
                ...
                if (! (this.hidden && this.hideable)) {
					if(this.pinned == 'right'){
						rh += this.width;
						if(o.vScrollBar.css('visibility') != 'hidden'){
							if(o.vScrollBar.css('width').indexOf('px') > -1){
								bar_w = Number(o.vScrollBar.css('width').split('px')[0]) + 5;
							}else{
            ...
            this.columnsheader.width(2 + h + rh);
        ...
        _rendercolumnheaders: function() {
            ...
            if (this.showeverpresentrow && this.everpresentrowposition !== "bottom") {
				...
				this.addnewrowtop.width(2 + h + rh);
                ...
                if (this.showeverpresentrow && this.everpresentrowposition === "bottom") {
					...
					this.addnewrowbottom.width(2 + h + rh);
						...
						if(g.pinned == 'right'){
							...
							if(this.vScrollBar.css('visibility') != 'hidden'){
								if(this.vScrollBar.css('width').indexOf('px') > -1){
									...
									scrollbar_w = Number(this.vScrollBar.css('width')) + 5;
								}
							}else{
								scrollbar_w = 0;
							}
        ...
        _updatescrollbarsafterrowsprerender: function() {
            ...
            var i = 0;
            var d = g.vScrollBar[0].style.visibility;	//垂直滚动条的是否出现
            if (d == "visible") {
                i = g.scrollbarsize + 3
            }
        ...
        _prerenderrows: function(o) {	//o = {visiblerecords: 14, virtualheight: 280}
			var C = this.that;
			/*减去多余宽度 */
            ...
            if (C.prerenderrequired == true) {
				...
				var A = C.columns.records.length;	//列头的长度
                if (b.jqx.browser.msie && b.jqx.browser.version > 8) {
				...
				// console.log(b.jqx.browser.version)	//版本
                var r = b.jqx.browser.msie && b.jqx.browser.version < 8; //false
                if (r) {
                    C.host.attr("hideFocus", "true")
                }
				var k = C.tableZIndex;	//	79
                if (u * A > k) {	// 10 * 7 > 79 => false 
                    k = u * A
                }
				var g = C.dataview.records.length == 0;		//false
							...
							if(w.pinned != 'right'){	//靠左边的+没有固定列的列头
								...
								if(C.vScrollBar[0].style.visibility != 'hidden'){
									...
									if((C.vScrollBar[0].style.width).indexOf('px') > -1){
										bar_w = Number((C.vScrollBar[0].style.width).split('px')[0]) + 5;
									}else{
										bar_w = Number(C.vScrollBar[0].css('width')) + 5;
									}
								}else{
									bar_w = 0;
								}
        ...
        _initializeColumns: function() {
            ...
            if (this.selectionmode == "checkbox") {		//复选框
                var g = new c(k, null);
                g.visibleindex = d++;
                g.width = k.groupindentwidth;
        ...
        _addHandlers: function() {
            ...
            if (!d) {
                ...
                e.addHandler(e.host, "keyup.edit" + e.element.id,
                function(k) {
                    if (k.keyCode === 9) {

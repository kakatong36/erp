...
(function(a) {
    a.extend(a.jqx._jqxGrid.prototype, {
        ...
        pasteselection: function() {
                ...
                if (B < g.length) {
                    ...
                    for (var z = 0; z < n.length; z++) {
                        ...
                        for (var y = 0; y < n[z].length; y++) {
                            ...
                            if (f.editable === false) { //如果列禁止编辑则跳过要粘贴的这行
                                y++;
                                continue
                            }
                            ...
                            if (f.cellvaluechanging) {
                                var b = f.cellvaluechanging(z, f.datafield, f.columntype, this.getcellvaluebyid(z, f.datafield), r);
                                if (b != undefined) {
                                    r = b
                                }
                            }
            /*
            if (f.validation) {//h: index, n: colName
                try {
                    var p = f.validation(e, r);
                    ...
                    if (!m) {
                        if (p.showmessage == undefined || p.showmessage == true) {
                            this._showvalidationpopup(m, f.datafield, l)
                        }
                        //this.editcell.validated = false;
                        return false
                    }
                } catch (r) {
                    this._showvalidationpopup(m, f.datafield, this.gridlocalization.validationstring);
                    //this.editcell.validated = false;
                    return false
                }
            }*/
                    ...
                    for (var l = 0; l < this._clipboardselection.length; l++) {
                        for (var C = 0; C < this._clipboardselection[l].length; C++) {
                            ...
                            if (r != null) {
                                ...
                                if (f.cellvaluechanging) {
                                    var b = f.cellvaluechanging(m, f.datafield, f.columntype, this.getcellvaluebyid(m, f.datafield), r);
                                    if (b != undefined) {
                                        r = b
                                    }
                                }
            ...
            if (f.validation) {//h: index, n: colName
                try {
                    let p = f.validation(e, r);
                    let l = this.gridlocalization.validationstring;
                    if (p.message != undefined) {
                        l = p.message
                    }
                    let m = typeof p == "boolean" ? p : p.result;
                    if (!m) {
                        if (p.showmessage == undefined || p.showmessage == true) {
                            this._showvalidationpopup(m, f.datafield, l)
                        }
